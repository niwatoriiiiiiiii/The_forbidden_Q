...
export class TimelinePlayer {
  ...
  private addForeground(x:number, y:number, texture:string) {
    ...
    // 画像サイズを指定
    foregroundImage.setDisplaySize(700, 700);
  }
  ...
  private setChoiceButtons(choices: Choice[]) {
    ...
    choices.forEach((choice, index) => {
      const y = (buttonGroupOriginY + buttonHeight * (index + 0.5) + buttonMargin * (index)) - 70;

      // Rectangleでボタンを作成
      const button = new Phaser.GameObjects.Rectangle(this.scene, width/2, y, width - buttonMargin*2, buttonHeight, 0x222222).setStrokeStyle(1, 0xffffff);
